...
style window:
    ...
    background Image("gui/textbox.png", xalign=0.9, yalign=1.0)
...
screen choice(items):
    ...
    timer 0.01 action Play('sound', "audio/fx3.mp3")
...
screen nav:
    ...
    on 'show' action Play('music', "audio/milk/milk31.mp3")
    
    add "gui/nav/mm.png" at mmz
    
    $lastsave=renpy.newest_slot(r"\d+")
    if lastsave is not None:
        $ name, page = lastsave.split("-")
        imagebutton auto "gui/nav/save_%s.png" xalign 0.5 ypos 514 action [Show("fake_save"), FileSave(1, confirm=False)] #hovered Play("sound", "narr.ogg") activate_sound "audio/click1.mp3"
    else:
        imagebutton auto "gui/nav/save_%s.png" xalign 0.5 ypos 514 action [Show("fake_save"), FileSave(1, confirm=False)]
screen fake_save:
    add "gui/nav/game saved.png"
    timer 1.5 action Hide('fake_save')
...
screen main_menu():
    ...
    add "gui/main menu/mm.png"
    
    $lastsave=renpy.newest_slot(r"\d+")
    if lastsave is not None:
        $ name, page = lastsave.split("-")
        imagebutton auto "gui/main menu/start_%s.png" focus_mask True action [Play("audio", "audio/start.mp3"), Start()] xpos 839 ypos 557  
    else:
        imagebutton auto "gui/main menu/start_%s.png" focus_mask True action [Play("audio", "audio/start.mp3"), Start()] xpos 839 ypos 557  
    ...
    if lastsave is not None:
        $ name, page = lastsave.split("-")
        imagebutton auto "gui/main menu/continue_%s.png" focus_mask True action FileLoad(name, page) xpos 524 ypos 557  #hovered Play("sound", "narr.ogg")
    else:
        imagebutton idle "gui/main menu/continue_inactive.png" action NullAction() xpos 524 ypos 557

    imagebutton auto "gui/main menu/exit_%s.png" focus_mask True action Quit(confirm=False) xpos 1253 ypos 557  #hovered Play("sound", "narr.ogg")
